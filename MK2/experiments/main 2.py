import pygame, os, cv2, sys, ctypes, random, math, multiprocessing, itertools
import numpy as np
...
def physics_calculator(input): #input is structured as follows: [originx, originy, vectorx, vectory]
    ...
    heightAtOrigin = pos[0] - (pos[1] / slope) # add formula for finding this based on the inputs
    mark = 1000000
    collisionPos = None
    ...
    if collisionPos is not None:
        for mirror in mirrors: # get the right mirror and use its data to determine the reflection
            if mirror.ID == mirrorId:
                ...
                raise SyntaxError("an error has come up with the mirror ID detection")
...
def main():
    ...
    while running:
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=cpuCoreCount) as executor:
            for result in  executor.map(physics_calculator, newArray):
                print(result)
        """
